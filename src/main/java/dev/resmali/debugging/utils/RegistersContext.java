...
public class RegistersContext {

    public static final Key<List<LazyValue<?>>> SMALI_LAZY_VALUES_KEY = Key.create("_smali_register_value_key_");

    public static PsiElement wrap(final Project project, final PsiElement originalContext) {
        ...
        return ApplicationManager.getApplication().runReadAction((Computable<PsiElement>) () -> {
            ...
            SmaliInstruction currentInstruction = (SmaliInstruction) PsiUtil.searchBackward(
                    originalContext,
            ...
            if (currentInstruction == null) {
                currentInstruction = (SmaliInstruction) PsiUtil.searchForward(
                        originalContext,
                        PsiMatchers.hasClass(SmaliInstruction.class),
                        PsiMatchers.hasClass(SmaliMethod.class)
                );
            ...
            int totalRegisters = containingMethod.getRegisterCount();
            int paramRegisters = containingMethod.getParameterRegisterCount();
            int firstParamRegistersIdx = totalRegisters - paramRegisters;
            Map<String, String> registerMap = Maps.newHashMap();
            ...
            TextWithImportsImpl textWithImports = new TextWithImportsImpl(
                    CodeFragmentKind.CODE_BLOCK,
                    variablesText.toString(),
                    "",
                    SmaliFileType.INSTANCE
            );
            JavaCodeFragment codeFragment = DefaultCodeFragmentFactory.getInstance()
                    .createPsiCodeFragmentImpl(textWithImports, originalContext, project);
            ...
            codeFragment.accept(new JavaRecursiveElementVisitor() {
                @Override public void visitLocalVariable(final @NotNull PsiLocalVariable variable) {
                    ...
                    if (registerMap.containsKey(name)) {
                        ...
                        LazyValue<?> lazyValue =
                                LazyValue.create(containingMethod, project, registerNumber, name, registerMap.get(name));
    ...
    private static void populateVisibleRegisters(
            Map<String, String> registerMap,
            StringBuilder variablesText,
            AnalyzedInstruction analyzedInstruction,
            int registersCount,
            int firstParameterRegister
    ) {
